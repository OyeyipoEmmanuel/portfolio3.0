...
const ProjectCard = ({ project }: { project: ProjectsDataType }) => {
    ...
    return (
        <div ref={cardRef} className="w-full grid grid-cols-1  gap-y-6 md:gap-y-3 rounded-[20px] overflow-hidden p-5 border-[1.5px] border-[#0000001a] relative transition-all ">
            <div
                ref={imageContainerRef}
                className="group relative w-full border border-[#00000015] rounded-[20px] overflow-hidden"
                onMouseEnter={() => {
                    if (project.videoSrc == null) return;
                    const video = videoRef.current;
                    if (!project.videoSrc && !video) return;
            ...
            >
                <Image
                    ...
                    className={`inset-0 w-full h-full object-cover z-10 transition-opacity duration-1000 ${project.videoSrc && "group-hover:opacity-0"}`}
                />
                {project.videoSrc && <video
                    ref={videoRef}
                    muted
                    loop
           ...
           <div className={`flex justify-end ${project.videoSrc == null && " invisible"}`}>
            ...
            <div ref={stackRef} className="grid grid-cols-2 md:grid-cols-3 gap-2 min-h-[80px]">
                {project.stack.map((stack, idx) => (
                    <span key={idx} className="px-4 group relative overflow-hidden py-2 border-2 h-fit border-black text-sm font-bold uppercase hover:text-white tracking-wide before:absolute before:inset-0 before:w-full before:h-full before:bg-black before:translate-y-[100%] hover:before:translate-y-0 before:transition-transform before:duration-500">
                        <p className="relative group-hover:text-white z-10 text-[12px] tracking-tighter md:text-md">{stack}</p>
                    </span>
                ))}
            </div>
